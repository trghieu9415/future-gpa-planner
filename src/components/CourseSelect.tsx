      ...
      <div className="mt-2 mb-4">
        ...
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" className="w-96 justify-between h-8" role="combobox" aria-expanded={popoverOpen}>
              {selectedOpenCourse ? (
            ...
            <TableBody className="divide-y divide-border">
              {selectedOpenCourse && selectedOpenCourse.groups.length > 0 ? (
                selectedOpenCourse.groups.map((group) => (
                  <>
                    <TableRow key={group.groupId} className="divide-x divide-border">
                    ...
                    {group.schedule.slice(1).map((scheduleItem, index) => (
                      <TableRow key={`${group.groupId}-${index}`} className="divide-x divide-border">
                        <TableCell className="whitespace-nowrap px-2 py-1.5 text-start">
